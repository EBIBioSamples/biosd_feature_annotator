...
//
//		btm.begin ();
//		EntityManager em = btm.getEntityManager ();
//		
...
//	}
	...
	/**
	 * Returns null, cause we never save entries in the DB at this stage.
	 */
	@Override
	public List<DiscoveredTerm> save ( String valueLabel, String typeLabel, List<DiscoveredTerm> dterms )
		throws OntologyDiscoveryException
	{
		if ( dterms.isEmpty () ) return CachedOntoTermDiscoverer.NULL_RESULT;
		...
		for ( int i = 0; i < dterms.size (); i++ )
		{
			...
			synchronized ( store )
			{
				...
				if ( oterm == null )
				{
					// Is it in the DB, then?
					OntologyEntry otermDb = ontoDao.find ( dtermUri, null, null, null );
					oterm = otermDb == null ? new OntologyEntry ( dtermUri, null ) : otermDb;
					// Save a new one then, whether it's from DB or completely new.
					store.find ( oterm, dtermUri );
				}
				...
				// cause the ontology entry might already exist (ie, a new annotation about a new string pair is being added) 
				//
				AnnotatorResources.getInstance ().getAnnNormalizer ().normalize ( zoomaMarker );
	
				// For an existing onto-term, this tells that a new string pair is associated to it. 
				oterm.addAnnotation ( zoomaMarker );
			}
			
			dterms.set ( i, new ExtendedDiscoveredTerm ( dterm.getUri (), dterm.getScore (), oterm ) );
	...
	public List<DiscoveredTerm> getOntologyTermUris ( String valueLabel, String typeLabel ) throws OntologyDiscoveryException
	{
		try
		{
			...
			for ( Object[] tuple: dbentries )
			{
				...
				DiscoveredTerm dterm = new DiscoveredTerm ( new URI ( oterm.getAcc () ),  score.floatValue () );
				result.add ( dterm );
			}
